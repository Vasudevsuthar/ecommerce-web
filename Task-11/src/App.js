...
function App() {
  ...
  const fetchMoviesHandler = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch('https://react-http-cd3dd-default-rtdb.firebaseio.com/movies.json');
      if (!response.ok) {
  ...
  async function onDeleteMovie(id){
    try{
      const movieData= await fetch(`https://react-http-cd3dd-default-rtdb.firebaseio.com/movies/${id}.json//`, {
        method: 'GET'
    });
    if(movieData.ok){
      ...
      const res= await fetch(`https://react-http-cd3dd-default-rtdb.firebaseio.com/movies/${id}.json//`, {
        method: 'DELETE'
    });
    if (res.ok) {
